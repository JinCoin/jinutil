...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := jinutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(jinutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := jinutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := jinutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := jinutil.NewBlockFromBytes(blockBytes)
	if err != nil {
