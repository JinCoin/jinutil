...
A Block defines a jincoin block that provides easier and more efficient
...
A Tx defines a jincoin transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Jincoin address.  While the
most common type is a pay-to-pubkey-hash, Jincoin already supports others and
...
To decode/encode an address:
	...
	addr, err := jinutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
