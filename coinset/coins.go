...
func (cs *CoinSet) TotalValue() (value jinutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue jinutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue jinutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount jinutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue jinutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue jinutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue jinutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue jinutil.Amount, coins []Coin) (Coins, error) {
...
// jinutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() jinutil.Amount {
	return jinutil.Amount(c.txOut().Value)
}
